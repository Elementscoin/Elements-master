...
const string strMessageMagic = "Elements Signed Message:\n";
...
{
    // Elements: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60 * 60; // Elements: 4 hours
static const int64 nTargetSpacing = 30; // Elements: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Elements: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // Elements: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Elements: 1 day
...
{
    // Elements: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Elements: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ElementsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ElementsMiner : ProcessBlock, block not accepted");
    }
